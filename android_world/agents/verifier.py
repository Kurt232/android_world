...
class CodeAgentConfig:
  ...
  def __init__(self, pre_code_path: str, code_path: str, xpath_path: str,
               api_path: str, dependency_path: str):
    self.pre_code_path = pre_code_path
  ...
  def data(self):
    ...
    code = load_txt_file(self.code_path)
    pre_code = load_txt_file(self.pre_code_path)

    api_xpaths = load_json_file(self.xpath_path)
    api_data = load_json_file(self.api_path)
    dependencies = load_json_file(self.dependency_path)

    return code, pre_code, api_xpaths, api_data, dependencies

  def check(self):
    assert os.path.exists(self.pre_code_path)
    assert os.path.exists(self.code_path)
    assert os.path.exists(self.xpath_path)
    assert os.path.exists(self.api_path)
    assert os.path.exists(self.dependency_path)


# ui_apis
api_names = [
    'tap', 'set_text', 'scroll', 'long_tap', 'get_text', 'get_attributes',
...
def regenerate_script(script, verifier_instant_name, device_instant_name,
                      input_policy_instant_name, api_xpaths_instant_name):
  '''
  ...
  modified_lines = [
      f'def autodroidv2_task_solution_code({verifier_instant_name}, {input_policy_instant_name}, {api_xpaths_instant_name}):'
  ...
  for original_line_num, line in enumerate(script_lines):
    ...
    if match:
      ...
      beginning_tabs = get_leading_tabs(line)
  ...
  modified_lines.append(
      f'autodroidv2_task_solution_code({verifier_instant_name}, {input_policy_instant_name}, {api_xpaths_instant_name})'
  )
  script = '\n'.join(modified_lines)
...
def save_current_ui_to_log(input_policy,
                           api_name,
                           currently_executing_code=None):
  log_path = os.path.join(input_policy.device.output_dir,
                          f'log_{input_policy.task_id}.yaml')
  current_state = input_policy.device.get_current_state()
  element_tree = input_policy.memory._memorize_state(
      current_state)['element_tree']
  state_desc = element_tree.get_str(is_color=False)

  if os.path.exists(log_path):
    output_log = load_yaml_file(log_path)
    ...
    if current_state.state_str != last_state_str:
      ...
      dump_yaml_file(log_path, output_log)
...
class ElementList:
  ...
  def __init__(self, api_name, api_xpath, input_policy, api_xpaths,
               verifier) -> None:
  ...
  def _save_getting_info_action(self, action_type, current_code_line,
    ...
    yaml_path = os.path.join(self.save_path,
                             f'log_{self.input_policy.task_id}.yaml')
    state = self.input_policy.device.get_current_state()
    element_tree = self.input_policy.memory._memorize_state(
        state)['element_tree']
    state_desc = element_tree.get_str(is_color=False)
  ...
  def check_api_name(self, api_name):
    if api_name not in self.api_xpaths.keys():
      ...
      code_lines = load_txt_file('tmp/compiled_code.txt').split('\n')
      # lines = original_script.split('\n')
      line_with_api_name = None
      for line_num, line in enumerate(code_lines):
        if api_name in line:
          line_with_api_name = line.strip()
          lineno_in_original_script = load_json_file('tmp/line_mappings.json')[
              str(line_num)]
          original_code_line = load_txt_file('tmp/combined_code.txt').split(
  ...
  def __getitem__(self, selector):
    ...
    lineno_in_original_script = int(
        load_json_file('tmp/line_mappings.json')[str(lineno - 1)])
    original_code_line = load_txt_file('tmp/combined_code.txt').split(
        '\n')[lineno_in_original_script]
  ...
  #     yaml_path = os.path.join(self.save_path, f'log_{self.input_policy.task_id}.yaml')
...
class Verifier:
  ...
  def check_output_crash(self, api_name):
    output_log = load_yaml_file(
        os.path.join(self.save_path,
                     f'log.yaml'))  # todo:: ignored log number
  ...
  def get_text(self, element_selector):
    ...
    if not target_ele:
      ...
      return element_tree.get_text(target_ele)
  ...
  def get_attributes(self, element_selector):
    ...
    # yaml_path = os.path.join(self.save_path, f'log_{self.input_manager.task_id}.yaml')
    # state = self.input_policy.device.get_current_state()
    ...
    if not target_ele:
      ...
      return target_ele.get_attributes()
  ...
  def back(self):
    ...
    old_state = self.input_policy.device.get_current_state()
    element_tree = self.input_policy.memory._memorize_state(
        old_state)['element_tree']
    state_desc = element_tree.get_str(is_color=False)
    back_event = {'action_type': 'navigate_back'}
